  ...
  */
  private readonly ensureDbResolved = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[ArmorModel] No se pudo autodetectar DB, usando valor por defecto:', this.dbName, (e as Error).message);
    } finally {
      this.resolved = true;
    }
  }
  ...
  readonly getAll = async (filters?: { heroType?: string; armorType?: string; status?: string | boolean; }): Promise<ArmorInterface[]> => {
