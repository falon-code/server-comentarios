...
export function signToken(payload: Omit<TokenPayload, 'exp'>, ttlSeconds = 7200, secret = process.env['AUTH_SECRET'] || 'dev-secret'): string {
...
export function verifyToken(token: string, secret = process.env['AUTH_SECRET'] || 'dev-secret'): TokenPayload | null {
  ...
  try {
    const json = JSON.parse(Buffer.from(p64.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf-8')) as TokenPayload;
    const now = Math.floor(Date.now() / 1000);
